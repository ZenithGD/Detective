...
class SparseDiffStage(Stage):
    ...
    def run(self, input: _SDInput, context : Pipeline) -> _SDOutput:
        ...
        kp_new_cv = [cv2.KeyPoint(x[0], x[1], 10) for x in kp_new]
        kp_old_cv = [cv2.KeyPoint(x[0], x[1], 10) for x in kp_old]
        ...
        img3 = cv2.drawMatchesKnn(context.target,kp_new_cv,context.target,kp_old_cv,matches,None,**draw_params)
        plt.imshow(img3)
        plt.show()
        return None